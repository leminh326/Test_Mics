...
static const char local_name[] = { AD_TYPE_COMPLETE_LOCAL_NAME ,'W','I','N','K','Y','-','F','E','F','E'};
...
static void Adv_Request(APP_BLE_ConnStatus_t New_Status)
{
	...
	char BoardName[12] = {AD_TYPE_COMPLETE_LOCAL_NAME ,'W','I','N','K','Y','-','X','X','X','X',0};
	const uint8_t *bd_addr;
	bd_addr = BleGetBdAddress();
	snprintf(BoardName+7, 5, "%02X%02X", bd_addr[1], bd_addr[0]);
	//snprintf(BoardName+9, 5, "%02X%02X", bdaddr[1], bdaddr[0]);
...
{
	UNUSED(argument);
	APP_DBG_MSG("HciUserEvtProcess STARTED!!!\r\n");
	for(;;)
	{
		osThreadFlagsWait( 1, osFlagsWaitAny, osWaitForever);
		hci_user_evt_proc( );
...
{
	UNUSED(flag);
	//APP_DBG_MSG("hci_cmd_resp_release SemHciId sem release\r\n");
...
{
	UNUSED(timeout);
	//APP_DBG_MSG("hci_cmd_resp_wait SemHciId semaphore acquire\r\n");
...
static void BLE_StatusNot( HCI_TL_CmdStatus_t status )
{
	...
	{
	case HCI_TL_CmdBusy:
		//APP_DBG_MSG("BLE_StatusNot HCI_TL_CmdBusy\r\n");

		osMutexAcquire( MtxHciId, osWaitForever );
		break;

	case HCI_TL_CmdAvailable:
		//APP_DBG_MSG("BLE_StatusNot HCI_TL_CmdAvailable\r\n");
